...
let dev_lock = false;
let dev_id = 0;
let dev_desc = null; // If any Passthru functions return JSON with 'err' in the key, then it is an API Error
...
function log_res(input) {
  if (input['err'] != null) {
    ...
    if (msg === "Unspecified error") {
      // Try to get the error string from library
...
ipc.on(consts.PT_CLOSE, event => {
  ...
  if (dev_lock) {
    // No DON'T close the connection - Something is being written or read!
    event.returnValue = {
      "err": "Device in operation"
    };
  } else {
