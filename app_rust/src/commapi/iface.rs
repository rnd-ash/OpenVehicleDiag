    ...
    pub fn get_param(&self, param_name: IFACE_CFG) -> InterfaceResult<u32> {
        match self.params.get(&param_name.to_string()) {
            ...
            None => Err(ComServerError {
                err_code: 999,
                err_desc: format!(
                    "Interface configuration has missing parameter {:?}",
                    param_name
                ),
            }),
        }
...
impl CanbusInterface {
    pub fn new(dev: Box<dyn ComServer>) -> InterfaceResult<Box<dyn Interface>> {
        if dev.get_capabilities().support_can_fd() != Capability::Yes {
            Err(ComServerError {
                err_code: 1,
                err_desc: "Device does not support CAN".into(),
            })
        } else {
            Ok(Box::new(CanbusInterface {
                dev: dev.clone_box(),
            }))
        }
    }
...
impl Interface for CanbusInterface {
    ...
    fn send_data(&mut self, data: &[InterfacePayload], timeout: u32) -> InterfaceResult<usize> {
        let can_packets: Vec<CanFrame> =
            data.iter().map(|f| CanFrame::new(f.id, &f.data)).collect();
...
impl IsoTPInterface {
    pub fn new(dev: Box<dyn ComServer>) -> InterfaceResult<Box<dyn Interface>> {
        if dev.get_capabilities().supports_iso15765() != Capability::Yes {
            Err(ComServerError {
                err_code: 1,
                err_desc: "Device does not support IsoTP".into(),
            })
        } else {
            Ok(Box::new(IsoTPInterface {
                dev: dev.clone_box(),
            }))
        }
    }
...
impl Interface for IsoTPInterface {
    ...
    fn send_data(&mut self, data: &[InterfacePayload], timeout: u32) -> InterfaceResult<usize> {
        let isotp_data: Vec<ISO15765Data> = data
            .iter()
            .map(|t| ISO15765Data {
                id: t.id,
                data: t.data.clone(),
...
impl Iso14230Interface {
    pub fn new(dev: Box<dyn ComServer>) -> InterfaceResult<Box<dyn Interface>> {
        if dev.get_capabilities().supports_iso14230() != Capability::Yes {
            Err(ComServerError {
                err_code: 1,
                err_desc: "Device does not support ISO14230".into(),
            })
        } else {
            Ok(Box::new(Iso14230Interface {
                dev: dev.clone_box(),
            }))
        }
    }
...
impl Iso9141Interface {
    pub fn new(dev: Box<dyn ComServer>) -> InterfaceResult<Box<dyn Interface>> {
        if dev.get_capabilities().supports_iso9141() != Capability::Yes {
            Err(ComServerError {
                err_code: 1,
                err_desc: "Device does not support ISO9141".into(),
            })
        } else {
            Ok(Box::new(Iso9141Interface {
                dev: dev.clone_box(),
            }))
        }
    }
...
impl DynamicInterface {
    ...
    pub fn exec<R, F: Fn(&mut Box<dyn Interface>) -> InterfaceResult<R>>(
        &mut self,
        func: F,
    ) -> InterfaceResult<R> {
        match self.iface.as_mut() {
            ...
            None => Err(ComServerError {
                err_code: 98,
                err_desc: "Dynamic interface not configured!".into(),
            }),
        }
    }
}
