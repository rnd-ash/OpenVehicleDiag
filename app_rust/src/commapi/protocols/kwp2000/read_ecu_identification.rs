...
use super::{KWP2000ECU, bcd_decode, bcd_decode_slice};
...
pub fn read_dcs_id(ecu: &KWP2000ECU) -> ProtocolResult<DcsEcuId> {
    ...
    if res.len() != 18 {
        return Err(ProtocolError::InvalidResponseSize { expect: 18, actual: res.len() })
    }
    Ok(DcsEcuId {
        ...
        production_date: format!("{}/{}/{}", bcd_decode(&res[17]), bcd_decode(&res[16]), bcd_decode(&res[15])),
    })
}
...
pub fn read_dcx_mmc_id(ecu: &KWP2000ECU) -> ProtocolResult<DcxMmcECUId> {
    ...
    if res.len() != 22 {
        return Err(ProtocolError::InvalidResponseSize { expect: 22, actual: res.len() })
    }
    Ok(DcxMmcECUId{
        ecu_origin: res[2],
        supplier_id: res[3],
...
fn read_fingerprint(ecu: &KWP2000ECU, cmd: u8) -> ProtocolResult<CodeFingerprint> {
    ...
    if res.len() < 4 {
        return Err(ProtocolError::InvalidResponseSize { expect: 4, actual: res.len() })
    }
