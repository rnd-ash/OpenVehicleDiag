...
impl DiagScanner {
                ...
                if let Err(e) = self.server.open_can_interface(500_000, false) {
                    ...
                    match self.server.add_can_filter(commapi::comm_api::FilterType::Pass, 0x0000, 0x0000) {
                        Ok(f_idx) => {
                            ...
                            // on most cars
                            if self.server.send_can_packets(&[CanFrame::new(0x07DF, &[0x09, 0x02])], 0).is_err() {
                                self.status = "Could not send wake up test packet".into();
                                self.server.close_can_interface().expect("What a terrible failure. Closing CAN Iface failed!?");
                            } else {
    ...
    fn poll(&mut self) -> Option<DiagScannerMessage> {
        match self.curr_stage {
            ...
            2 => {
                if self.curr_scan_id >= 0x7FF { // Scanning complete
                    Some(DiagScannerMessage::IncrementStage)
                } else if self.clock.elapsed().as_millis() >= 100 { // Timeout waiting for response
                    self.get_next_canid();
                    self.server.clear_can_rx_buffer();
                    ...
                    self.server.send_can_packets(&[CanFrame::new(self.curr_scan_id, &[0x10, 0x10, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00])], 0);
                    // Reset the timer
                    ...
                    for frame in &self.server.read_can_packets(0, 10000).unwrap_or_default() {
                        if self.can_traffic_id_list.get(&frame.id).is_none() { // Its a new frame we haven't seen before!
                            ...
                            if payload[0] == 0x30 && payload.len() == 8 {
                                ...
                                if let Some(r) = self.stage2_results.get_mut(&self.curr_scan_id) {
                                    ...
                                    self.stage2_results.insert(self.curr_scan_id, vec![frame.id]);
                                }
                            }
                        }
                    }
            ...
            3 => {
                if self.clock.elapsed().as_millis() > 100 {
                    ...
                    let filter_id = self.stage2_results.get(&keys[self.curr_scan_id as usize]).unwrap();
                    self.filter_idx = self.server.add_can_filter(commapi::comm_api::FilterType::Pass, filter_id[0], 0xFFFF).unwrap();
                    self.server.send_can_packets(&[CanFrame::new(keys[self.curr_scan_id as usize], &[0x10, 0x10, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00])], 0);
                    self.clock = Instant::now();
                    ...
                    for frame in &self.server.read_can_packets(0, 10000).unwrap_or_default() {
                        let payload = frame.get_data();
                        if payload[0] == 0x30 && payload.len() == 8 { // Not a false positive! We can add the Config to list!
                            self.stage3_results.push(
                                ISO15765Config {
        ...
        match msg {
            ...
            DiagScannerMessage::SaveResults => {
                ...
                let mut file = match File::create(&path) {
                    Ok(j) => j,
                    Err(e) => {
                        self.status = format!("Error creating save file {}: {}",path.as_os_str().to_str().unwrap(),  e);
                        return None
                    }
                };
                match file.write_all(json.as_bytes()) {
                    ...
                    Err(e) => {
                        self.status = format!("Error writing to save file {}: {}",path.as_os_str().to_str().unwrap(),  e);
                    }
                }
                None
            }
    ...
    fn draw_stage_0(&mut self) -> Element<DiagScannerMessage> {
        let mut c = Column::new().padding(10).spacing(10).align_items(Align::Start).width(Length::Fill)
            ...
            if !self.status.is_empty() {
                c = c.push(
                    Row::new().width(Length::Fill).align_items(Align::Center))
                    .push(text(format!("Could not start scanning: {}", self.status).as_str(), TextType::Danger))
            }
            c.into()
    }
    ...
    fn draw_stage_2(&mut self) -> Element<DiagScannerMessage> {
        ...
        .push(title_text("Scanning for ISO-TP ECUs (Stage 1/2)", crate::themes::TitleSize::P2))
        .push(progress_bar(0f32..=0x7FF as f32, self.curr_scan_id as f32, ButtonType::Info))
        .push(text(format!("Testing CAN ID 0x{:04X}", self.curr_scan_id).as_str(), TextType::Normal))
        .push(Space::with_height(Length::Units(20)));

        for (id, ls) in &self.stage2_results {
            c = c.push(text(format!("Found {} potential Receiver IDs for Sender 0x{:04X}", ls.len(), id).as_str(), TextType::Normal))
        }
        if !self.status.is_empty() {
            c = c.push(
                Row::new().width(Length::Fill).align_items(Align::Center))
                .push(text(format!("Error scanning: {}", self.status).as_str(), TextType::Danger))
        }
        c.into()
    }
    ...
    fn draw_stage_3(&mut self) -> Element<DiagScannerMessage> {
        let mut c = Column::new().padding(10).spacing(10).align_items(Align::Center).width(Length::Fill)
        .push(title_text("Scanning for ISO-TP ECUs (Stage 2/2)", crate::themes::TitleSize::P2))
        .push(progress_bar(0f32..=self.stage2_results.len() as f32, self.curr_scan_id as f32, ButtonType::Info))
    ...
    fn draw_stage_7(&mut self) -> Element<DiagScannerMessage> {
        ...
        for ecu in &self.stage4_results {
            c = c.push(text(format!("ECU 0x{:04X} - KWP2000?: {}, UDS?: {}", ecu.send_id, ecu.kwp_support, ecu.uds_support).as_str(), TextType::Normal));
        }

        // Allow the user to save the results to file
        if self.stage4_results.is_empty() {
            c = c.push(text("Unfortunately, no ISO-TP capable ECUs were found in your vehicle", TextType::Normal));
        } else {
            c = c.push(text("Save results to file:", TextType::Normal));
            c = c.push(button_outlined(&mut self.btn, "Save results to file", ButtonType::Info).on_press(DiagScannerMessage::SaveResults));
            if self.save_attempted && !self.save_path.is_empty() {
                c = c.push(text(format!("Success, results saved to {}", self.save_path).as_str(), TextType::Success));
                c = c.push(text("You may now return home", TextType::Normal))
            } else if self.save_attempted && self.save_path.is_empty() && !self.status.is_empty() { // Error saving
                c = c.push(text(format!("Error saving results: {}", self.status).as_str(), TextType::Warning));
            }
        }
        c.into()
    }
    ...
    fn draw_stage_unk(&mut self) -> Element<DiagScannerMessage> {
        Row::new()
            .push(text(format!("Stage {} is unimplemented. Please report this!", self.curr_stage).as_str(), TextType::Normal))
            .into()
    }
}